#!/usr/bin/env python3
import re

# ——— Paste the entire “font[] = { … };” data here (just the hex bytes) ———
raw = """

    , // a-umlaut
    0x21, 0x55, 0x54, 0x78, 0x40, 0x20, 0x54, 0x55, 0x79, 0x40, 0x0C, 0x1E,
    0x52, 0x72, 0x12, 0x39, 0x55, 0x55, 0x55, 0x59, 0x39, 0x54, 0x54, 0x54,
    0x59, 0x39, 0x55, 0x54, 0x54, 0x58, 0x00, 0x00, 0x45, 0x7C, 0x41, 0x00,
    0x02, 0x45, 0x7D, 0x42, 0x00, 0x01, 0x45, 0x7C, 0x40, 0x7D, 0x12, 0x11,
    0x12, 0x7D, // A-umlaut
    0xF0, 0x28, 0x25, 0x28, 0xF0, 0x7C, 0x54, 0x55, 0x45, 0x00, 0x20, 0x54,
    0x54, 0x7C, 0x54, 0x7C, 0x0A, 0x09, 0x7F, 0x49, 0x32, 0x49, 0x49, 0x49,
    0x32, 0x3A, 0x44, 0x44, 0x44, 0x3A, // o-umlaut
    0x32, 0x4A, 0x48, 0x48, 0x30, 0x3A, 0x41, 0x41, 0x21, 0x7A, 0x3A, 0x42,
    0x40, 0x20, 0x78, 0x00, 0x9D, 0xA0, 0xA0, 0x7D, 0x3D, 0x42, 0x42, 0x42,
    0x3D, // O-umlaut
    0x3D, 0x40, 0x40, 0x40, 0x3D, 0x3C, 0x24, 0xFF, 0x24, 0x24, 0x48, 0x7E,
    0x49, 0x43, 0x66, 0x2B, 0x2F, 0xFC, 0x2F, 0x2B, 0xFF, 0x09, 0x29, 0xF6,
    0x20, 0xC0, 0x88, 0x7E, 0x09, 0x03, 0x20, 0x54, 0x54, 0x79, 0x41, 0x00,
    0x00, 0x44, 0x7D, 0x41, 0x30, 0x48, 0x48, 0x4A, 0x32, 0x38, 0x40, 0x40,
    0x22, 0x7A, 0x00, 0x7A, 0x0A, 0x0A, 0x72, 0x7D, 0x0D, 0x19, 0x31, 0x7D,
    0x26, 0x29, 0x29, 0x2F, 0x28, 0x26, 0x29, 0x29, 0x29, 0x26, 0x30, 0x48,
    0x4D, 0x40, 0x20, 0x38, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08,
    0x38, 0x2F, 0x10, 0xC8, 0xAC, 0xBA, 0x2F, 0x10, 0x28, 0x34, 0xFA, 0x00,
    0x00, 0x7B, 0x00, 0x00, 0x08, 0x14, 0x2A, 0x14, 0x22, 0x22, 0x14, 0x2A,
    0x14, 0x08, 0x55, 0x00, 0x55, 0x00, 0x55, // #176 (25% block) missing in old
                                              // code
    0xAA, 0x55, 0xAA, 0x55, 0xAA,             // 50% block
    0xFF, 0x55, 0xFF, 0x55, 0xFF,             // 75% block
    , // sharp-s or beta
     // #255 NBSP

"""

# 1. Extract all the 0x__ values
vals = [int(x, 16) for x in re.findall(r'0x[0-9A-Fa-f]{2}', raw)]
if len(vals) % 5 != 0:
    raise SystemExit(f"Expected a multiple of 5 bytes; got {len(vals)}")
chars = len(vals) // 5

# 2. BDF header (we’ll pretend each glyph is 5×7 pixels, no anti‐alias)
lines = [
    "STARTFONT 2.1",
    "FONT -Misc-Fixed-Medium-R-Normal--7-70-75-75-C-50-ISO10646-1",
    f"CHARS {chars}"
]

# 3. For each character code 0..(chars-1), emit a BDF glyph
for code in range(chars):
    glyph_bytes = vals[code*5 : code*5 + 5]
    lines.append(f"STARTCHAR U+{code:04X}")
    lines.append("ENCODING %d" % code)
    lines.append("SWIDTH 500 0")
    lines.append("DWIDTH 6 0")
    # BBOX: width=5, height=7, xoff=0, yoff=0
    lines.append("BBX 5 7 0 0")
    lines.append("BITMAP")
    # Each of the 7 rows: we need to assemble a 5-bit row from the 8-bit column bytes
    for row in range(7, -1, -1):  # from top (bit 6) down to bit 0, but only 7 rows
        row_bits = 0
        for col in range(5):
            if (glyph_bytes[col] >> row) & 0x01:
                row_bits |= (1 << (4 - col))
        # Print as two hex digits
        lines.append(f"{row_bits:02X}")
    lines.append("ENDCHAR")

lines.append("ENDFONT")

# 4. Write to glcdfont.bdf
with open("glcdfont.bdf", "w") as f:
    f.write("\n".join(lines))

print("Created glcdfont.bdf with %d glyphs" % chars)
